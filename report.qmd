...
fig_comp <- rents_df |>
  ...
  labs(x = element_blank(),y = "Median Rent",colour = element_blank(),shape=element_blank(),
       title="Median Rent of the Inner West LGA, Greater Sydney\nRegion, and Whole of NSW")

fig_comp
```

Median rent in the Inner West and Greater Sydney look somewhat different prior to the pandemic period, as seen in @fig-iw-syd-nsw-rent. During the pandemic, the trends seem to converge. 
...
df <- inner_west_rents |> 
  dplyr::filter(dwelling_type == "Total" & no_of_bedrooms != "Total" & no_of_bedrooms != "Not Specified")
df$no_of_bedrooms <- factor(df$no_of_bedrooms,levels=c("Bedsitter","1 Bedroom","2 Bedrooms","3 Bedrooms","4 or more Bedrooms"))

fig_rent_rooms <- df |>
  ggplot() +
  aes(x = date, y = median_rent_price, colour = no_of_bedrooms) +
  geom_line() +
  scale_x_date(breaks = scale,date_labels = "%b %Y") +
  scale_y_continuous(labels=scales::dollar_format()) +
  geom_vline(xintercept=as.Date("2020-03-15"),linetype="dotted") +
  geom_vline(xintercept=as.Date("2021-10-11"),linetype="dotted") +
  annotate("text",label="Pandemic\nPeriod",x=as.Date("2021-01-01"),y=1250) +
  theme(axis.text.x=element_text(angle = 45, vjust=1,hjust=1),plot.title = element_text(hjust = 0.5),
    legend.position = "bottom") +
  labs(x = element_blank(),y = "Median Rent",
       title = "Median Rent of the Inner West LGA\n(Factored by Number of Bedrooms)",
...
fig_iw_bonds_bedrooms <- inner_west_rents |>
  ...
  labs(fill=element_blank(),
       x = element_blank(),
       y= element_blank(),
       title="Total Number of Bonds by Number of Bedrooms\n(Quarter Ending June 2024)") +
  theme(plot.title = element_text(hjust = 0.5),legend.position="none")

fig_iw_bonds_bedrooms
```
...
fig_iw_bonds_dwelling <- inner_west_rents |>
  ...
  labs(fill=element_blank(),
       x = element_blank(),
       y= "Number of Bonds",
       title="Total Number of Bonds by Dwelling Type\n(Quarter Ending June 2024)") +
  theme(plot.title = element_text(hjust = 0.5),legend.position = "none")
...
temp_df = inner_west_rents |> 
  dplyr::filter(dwelling_type == "Total" & no_of_bedrooms == "Total")

cor(temp_df$cash_rate,temp_df$median_rent_price)
```

The median rent in the Inner West against cash rate is strongly correlated, with a Pearson correlation coefficient of 0.96. 

```{r}
#| label: fig-iw-bonds-against-cr
#| fig-cap: "Median rent in the Inner West LGA against the RBA cash rate"

fig_iw_rent_cr <- inner_west_rents |> 
  dplyr::filter(dwelling_type == "Total" & no_of_bedrooms == "Total") |>
  ggplot() +
  aes(x = cash_rate, y = no_of_bonds_total) +
  scale_y_continuous(labels=scales::comma) +
  scale_x_continuous(breaks=seq(0,4.5,by=0.5)) +
  geom_point(colour = innerwest_colour) +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(x = "Cash Rate",y = "Total Bonds",title = "Total Number of Bonds Held in the\nInner West LGA against RBA Cash Rate")
fig_iw_rent_cr
```

```{r}
cor(temp_df$cash_rate,temp_df$no_of_bonds_total)
```

The correlation between total number of bonds held and cash rate isn't as strong, with a correlation coefficient of 0.63.
...
fig_iw_sales_cr <- df |>
  ggplot() +
  aes(x = cash_rate, y = median_sales_price, colour = factor(year(date))) +
  scale_y_continuous(labels=scales::dollar_format()) +
  scale_x_continuous(breaks=seq(0,4.5,by=0.5)) +
  geom_point(colour = innerwest_colour) +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(x = "Cash Rate",y = "Median Sales Price (000's AUD)",title = "Median Sales Price in the\nInner West against Cash Rate")
...
cor(df$cash_rate,df$median_sales_price)

df |> filter(cash_rate < 2)
```

Correlation between median sales price and RBA cash rate is relatively weak, with a correlation coefficient of 0.18. The six quarters starting from the quarter ending September 2022 show some potential correlation, these are the points in @fig-sales-cr where the cash rate was greater than 2%. 

```{r}
#| label: fig-no-sales-cr

fig_iw_no_sales_cr <- df |>
  ggplot() +
  aes(x = cash_rate, y = no_of_sales, colour = factor(year(date))) +
  scale_x_continuous(breaks=seq(0,4.5,by=0.5)) +
  geom_point(colour = innerwest_colour) +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(x = "Cash Rate",y = element_blank(),title = "Median Sales Price in the\nInner West against Cash Rate")
...
cor(df$cash_rate,df$no_of_sales)

```

Number of properties sold is also weakly correlated with a correlation coefficient of -0.13
